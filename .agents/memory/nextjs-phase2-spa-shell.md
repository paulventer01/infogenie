...
Migrated React panels must never render `new Date().toLocale*()`, `Date.now()`, or other server/client-divergent values directly in JSX or in `useState`/`useMemo` initializers — server (Node) locale/time differs from the browser, producing different HTML and a hydration crash. The root `<html>` in `app/layout.tsx` carries `suppressHydrationWarning` because the pre-hydration themeInit script sets `data-theme`.

**The durable fix is at the boundary, not per-component:** `MigratedPanel` renders its feature component **client-only** via a mount guard (`useState(false)` + `useEffect(()=>setMounted(true),[])`; `return null` until mounted). Server and initial client render both emit nothing for the panel, then the real component mounts post-hydration — this eliminates the *entire* class (locale dates, `Date.now`/`Math.random`, window/localStorage reads) for all 200+ panels at once. **Why:** auditing every ported component for unsafe render values is infeasible and regresses every time a new screen is rebuilt; these panels are authed and load data client-side, so SSR adds no value. Prefer this over chasing `suppressHydrationWarning` into each component. The shell at `/` (Navbar/LegacyBody/SpaRouter/LegacyScripts/AccountMenu) is hydration-safe on its own — null/static/dangerouslySetInnerHTML/effect-only — so it needs no per-element suppression.
