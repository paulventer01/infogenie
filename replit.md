...
*   **AI Video — Storyboard + Key Frames (Creator Studio → AI Video Storyboard)**: Storyboard endpoint (`/api/studio/video/storyboard`) returns scene-by-scene VO + visuals. New `/api/studio/video/render-frames` endpoint loops each scene through Cloudflare SDXL to produce one key image per scene — users get a stitchable filmstrip without Runway/Veo/Kling. Frames persisted back onto the storyboard kv row.
