...
const FIELDS = Object.freeze({
  campaign_budget: Object.freeze(['campaign_budget.status', 'campaign_budget.resource_name']),
  campaign: Object.freeze(['campaign.status', 'campaign.resource_name', 'campaign.campaign_budget']),
  ad_group: Object.freeze(['ad_group.status', 'ad_group.resource_name', 'ad_group.campaign']),
});
const RESULT_KEYS = Object.freeze({
  campaign_budget: 'campaignBudget', campaign: 'campaign', ad_group: 'adGroup',
});
...
function defaultTransport(options) {
  ...
  if (url.origin !== API_ORIGIN || options.method !== 'POST'
    || !new RegExp(`^/${API_VERSION}/customers/[0-9]{10}/googleAds:search$`).test(url.pathname)
    || url.search) invalid('unsafe_google_ads_request');
  return new Promise((resolve) => {
    let settled = false;
    const finish = (value) => { if (!settled) { settled = true; resolve(value); } };
    const req = https.request(url, {
      method: 'POST', headers: options.headers, timeout: options.timeoutMs,
    }, (res) => {
...
function normalize(kind, body, bound, now) {
  ...
  if (body.results.length === 0) return failure(kind, { status: 404 }, now);
  if (body.results.length !== 1 || !body.results[0] || typeof body.results[0] !== 'object') {
    return failure(kind, { malformed: true }, now);
  }
  const resource = body.results[0][RESULT_KEYS[kind]];
  if (!resource || typeof resource !== 'object' || Array.isArray(resource)) {
    return failure(kind, { malformed: true }, now);
  }
  const parsed = parseResource(resource.resourceName, COLLECTIONS[kind]);
  if (!parsed || parsed.id !== bound.byKind[kind].provider_object_id) {
    return failure(kind, { malformed: true }, now);
  }
  const out = baseObservation(kind, now);
  out.outcome = 'observed';
  out.status_classification = statusClass(resource.status);
  out.account_binding_matches = parsed.customer === bound.customer_id;
  if (kind === 'campaign') {
    const parent = parseResource(resource.campaignBudget, COLLECTIONS.campaign_budget);
    out.budget_parent_matches = parent
      ? parent.customer === bound.customer_id
        && parent.id === bound.byKind.campaign_budget.provider_object_id : 'unknown';
  }
  if (kind === 'ad_group') {
    const parent = parseResource(resource.campaign, COLLECTIONS.campaign);
...
async function observePausedGoogleAdsLedger(input) {
  ...
  for (const kind of OBJECT_KINDS) {
    const url = new URL(`/${API_VERSION}/customers/${bound.customer_id}/googleAds:search`, API_ORIGIN);
    const resourceName = `customers/${bound.customer_id}/${COLLECTIONS[kind]}/${bound.byKind[kind].provider_object_id}`;
    const query = `SELECT ${FIELDS[kind].join(', ')} FROM ${kind} WHERE ${kind}.resource_name = '${resourceName}' LIMIT 1`;
    let response;
    try {
      response = await send({
        url: url.toString(), method: 'POST', headers: headers(bound),
