...
function buildReport(client, profile, data, workspaceBrand, selectedMetrics, dateRange) {
  ...
  const periodDetail = dateRange?.startDate ? `${dateRange.startDate} to ${dateRange.endDate}` : 'All mapped activity';
  table('Report scope', ['Item', 'Coverage'], [
    ['Client', client.name], ['Source', data.source], ['Period', periodLabel],
    ['Queried dates', periodDetail], ['Timezone', dateRange?.timezone || 'UTC'],
    ['Summary', data.summary_scope === 'all_mapped_records' ? 'All currently mapped records' : 'Mapped records with activity in period'],
    ['Mapped records', summary.mapped_records],
    ['Record list', `First ${records.length} of ${summary.mapped_records}; maximum 100`],
    ['Recent lists', 'Latest 50 per list within the queried period; not full history'],
