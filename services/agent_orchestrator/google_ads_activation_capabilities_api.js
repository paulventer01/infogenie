...
function route(label,fn){return async(req,res)=>{try{if(!human(req))throw service._deny('human_session_required');if(!grant(req))throw service._deny('permission_denied');
 const tenantId=await tenantCtx.resolveTenantId(req,{label:`google-ads-activation-capability:${label}`});const out=await fn(common(req,tenantId),req);res.status(label==='issue'&&!out.replay?201:200).json(out);
 }catch(e){res.status(status(e.code)).json({error:e.code||'capability_request_failed',external_action_taken:false});}};}
router.post('/',json,route('issue',(base,req)=>{exact(req.body,['reconciliation_run_id','confirmation_id','confirmation','ttl_ms'],['reconciliation_run_id','confirmation_id','confirmation']);return tx(c=>service.issue(c,{...base,reconciliationRunId:req.body.reconciliation_run_id,confirmationId:req.body.confirmation_id,confirmation:req.body.confirmation,ttlMs:req.body.ttl_ms}));}));
...
module.exports=router;module.exports._human=human;module.exports._grant=grant;
