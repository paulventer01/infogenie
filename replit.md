...
*   **Brandwatch-Killer Suite — Tiers 1-9 (compressed index)** — full design notes in git history; each entry: route · key endpoint · storage. All follow the same pattern (strict-JSON LLM prompt + `/^_DUMMY/i` key gate + template fallback + Postgres persistence + `_escapeHtml`/`_safeUrl` frontend builder).
...
*   **Competitor Tech Stack Detector (Compete → Tech Stack Detector, Tier 10 #1)**: BuiltWith-powered detector under `services/tech_stack/api.js`. `POST /api/tech-stack/detect {domain}` calls BuiltWith Free API and normalises into `{categories:[{category, live, dead, technologies:[{name, tag, first_detected, last_detected, live}]}], total}`. `POST /api/tech-stack/compare {domains:[1-5]}` runs all in parallel and builds a tech-presence matrix sorted by overlap count. Domain normalised (lowercased, strips `https?://`, `www.`, paths). Falls back to placeholder + helpful note when `BUILTWITH_API_KEY` missing/dummy. Frontend `buildTechStack()` shows two stacked forms — single-domain (full breakdown card grid with category panels and live/dead-coloured tech pills) + multi-domain compare (sortable matrix table with ✓/· presence cells per domain).
*   **AI Cold Email Writer (Reach → Cold Email Writer, Tier 10 #2)**: Multi-step cold outreach drafter under `services/cold_email/` (schema, api). Single table `cold_email_runs` (sender_brand, sender_name, sender_offer, target_company, target_role, target_pain, tone, sequence_steps, emails JSONB, generated_by, created_at). `POST /api/cold-email/generate {sender_*, target_*, tone, steps:1-5}` calls GPT-4o-mini with strict-JSON schema (`{emails:[{step, days_after_prev, subject, preview, body, cta, why_this_works}]}`); subjects must be lowercase-first plain English (no spam triggers), bodies 60-130 words, no buzzwords, cite ONE proof point. Step 1 opener, mid-steps escalate value, final step is a polite breakup. tone allowlist `direct|warm|consultative|witty|executive|curious`. Falls back to deterministic 5-step template (opener → loom follow-up → social-proof → close-the-loop → breakup) when OpenAI key missing/dummy. `GET /api/cold-email/history` and `GET /api/cold-email/:id`. Frontend `buildColdEmail()` shows compact form (your brand+name, offer textarea, target company+role, pain textarea, tone+steps dropdowns) + per-step cards with red-numbered avatar, subject in bold, italic preview, monospace body panel, red CTA line, dashed-border "💡 Why this works" rationale, and per-step 📋 Copy.
*   **Meta Ads Insights (Optimize → Meta Ads Insights, Tier 13 #1)**: Live Meta Marketing API dashboard under `services/meta_ads_insights/api.js`. Four endpoints — `POST /test`, `GET /account-summary?date_preset=`, `GET /campaigns?date_preset=`, `GET /top-ads?date_preset=` — all hit Graph API v19.0 `/act_{id}/insights` with allowlisted date presets (`today|yesterday|last_7d|last_14d|last_30d|last_90d|this_month|last_month`). Auto-prefixes account id with `act_` if missing. Parses purchases + revenue from `actions`/`action_values` (matches both `purchase` and `omni_purchase`), computes ROAS = revenue/spend (2dp). Top-ads sorted by CTR desc, top 15. `_friendlyError()` translates Meta scope errors into actionable hints (add `ads_read` from a System User in Business Settings). `_hasCreds()` enforces `^_DUMMY` gate. Frontend `buildMetaInsights()` shows blue Meta gradient header with date-preset dropdown + ⚡ Test, 11-tile KPI grid (spend/revenue/ROAS coloured ≥2 green / ≥1 amber / <1 red, plus impressions/reach/clicks/CTR/CPC/CPM/frequency/purchases), sortable campaign table (ROAS pill same colours), and creative cards grid sorted by CTR with green/blue/amber pills.
*   **Keyword Explorer (Plan → Keyword Explorer, Tier 13 #2)**: DataForSEO Labs keyword research tool under `services/keyword_explorer/` (schema, api). Single table `keyword_explorer_runs` (seed, country, seed_metrics JSONB, ideas JSONB, ran_at). `POST /explore {seed, country, limit:5-50}` runs `/v3/dataforseo_labs/google/keyword_overview/live` + `/keyword_ideas/live` IN PARALLEL — overview returns seed metrics (search_volume, cpc, competition_level, keyword_difficulty 0-100, intent informational|navigational|commercial|transactional, top-of-page bid range), ideas returns up to 50 related kw sorted by volume desc with same metrics each. 15-country location_code map shared with SERP Tracker. `^_DUMMY` gate. `GET /history` (last 30 runs) + `GET /:id`. Frontend `buildKeywordExplorer()` shows compact 4-field form (seed + country + ideas count + 🔬 Explore button) → purple gradient seed header with intent line + 6-tile metrics row (KD coloured ≤30 green / ≤60 amber / >60 red) + sortable ideas table with KD pill / volume / green CPC / competition / intent + 📥 Export CSV button.
*   **SERP Position Tracker (Compete → SERP Tracker, Tier 12 #1)**: DataForSEO-powered Google rank tracker under `services/serp_tracker/` (schema, api). Two tables: `serp_tracker_keywords` (keyword+target_domain+country UNIQUE, enabled) and `serp_tracker_runs` (target_position, target_url, total_results, results JSONB, ran_at). `POST /scan/:id` calls DataForSEO `/v3/serp/google/organic/live/regular` (depth=10, country→location_code map for 15 markets), filters to organic results only, finds target's exact rank by domain match (handles `nike.com` vs `www.nike.com` vs `running.nike.com`). REST CRUD `/api/serp-tracker/{keywords,scan/:id,scan-all,history/:id}`. (Note: started on Google Custom Search but the project's GOOGLE_SEARCH_API_KEY doesn't have CSE JSON API enabled — pivoted to DataForSEO which is already used by Backlinks/Search-Pulse.) Frontend `buildSerpTracker()` shows compact add-keyword form (keyword + domain + country) + sortable table (position pill green ≤3 / amber ≤10 / red >10) with ⚡ Scan / 📈 History (inline scan-by-scan timeline with target URL link) / 🗑 Delete + ⚡ Scan All button.
*   **HubSpot Sync (Reach → HubSpot Sync, Tier 12 #2)**: HubSpot CRM bridge under `services/hubspot_sync/api.js`. Five endpoints: `POST /test` (GET `/crm/v3/objects/contacts?limit=1` to verify token), `POST /push-lead {lead}` (single contact upsert by email via `idProperty=email`), `POST /push-influencer {influencer}` (creates from creator object), `POST /push-bulk {leads[]}` (batch upsert max 50/call via `/contacts/batch/upsert`), `GET /recent-contacts?limit=N` (last 50 contacts sorted by createdate desc). All auth via `HUBSPOT_PRIVATE_APP_TOKEN` Bearer header. Maps lead → standard HS props (firstname, lastname, email, jobtitle, company, website, city, industry, hs_lead_status='NEW', lifecyclestage='lead'). Splits full name on first whitespace. On scope errors, returns clear hint: "Add the 'crm.objects.contacts.write' scope to your HubSpot Private App." Frontend `buildHubspotSync()` shows orange HubSpot gradient header with ⚡ Test button + 2-column form (📥 push last B2B Lead Finder run via `window._lfLeads` cache · ✏️ single-contact form) + recent-contacts table with name/email/title/company/lifecycle pill. (Read works out-of-the-box with `crm.objects.contacts.read` scope; write requires adding `crm.objects.contacts.write`.)
*   **Web Vitals Auditor (Compete → Web Vitals Auditor, Tier 11 #1)**: Google PageSpeed Insights v5 wrapper at `services/web_vitals/api.js`. `POST /api/web-vitals/audit {url}` runs Lighthouse on mobile + desktop in parallel (`category=performance|accessibility|best-practices|seo`). Normalises Lighthouse scores (0-100), lab metrics (LCP, FCP, CLS, TBT, TTI, SI with good/bad threshold flags per Web Vitals spec), real-user CrUX field data (LCP/CLS/INP percentiles + FAST/AVERAGE/SLOW category), and top 6 opportunities (sorted by `numericValue` savings ms). 60s per-strategy timeout. Falls back to placeholder when `GOOGLE_PAGESPEED_API_KEY` missing. Frontend `buildWebVitals()` shows URL input + side-by-side mobile/desktop cards with indigo gradient header, 4-score tile bar (perf/a11y/bp/seo coloured by 90/50/0 thresholds), 6-metric Web Vitals grid (green if good, amber otherwise), CrUX field-data row with FAST/AVERAGE/SLOW left border, and amber "TOP OPPORTUNITIES" panel showing Lighthouse savings.
*   **B2B Lead Finder (Reach → B2B Lead Finder, Tier 11 #2)**: Perplexity-powered B2B prospect researcher under `services/lead_finder/` (schema, api). Single table `lead_finder_runs` (query_filters JSONB, result_count, leads JSONB, source, created_at). `POST /api/lead-finder/search {titles[], seniorities[], locations[], industries[], company_size_min, company_size_max, per_page:3-15}` calls Perplexity sonar with strict-JSON system prompt asking for real currently-active prospects from public sources (LinkedIn/press/news), `{leads:[{name, title, seniority, department, company, company_domain, company_size, company_industry, location, linkedin_url, email_status:'unknown', email:'', why_fit, source_url}]}`. NEVER invents emails (always blank, status=unknown). Max 2 leads per company for diversity. (Note: started on Apollo but free plan blocks `mixed_people/search`/`organizations/search`/`people/match` — pivoted to Perplexity which is more flexible and uses an API key already wired up.) `GET /api/lead-finder/history` and `GET /api/lead-finder/:id`. Frontend `buildLeadFinder()` shows compact form (titles comma-sep + multi-select seniorities + locations + company size min/max) + lead card grid (auto-fill 380px) with avatar, name + title + indigo company line, location/industry/email-status pills, 💼 LinkedIn (LinkedIn blue) + 🌐 Site buttons, and 📥 Export CSV.
