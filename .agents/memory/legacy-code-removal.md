...
**Shared-helper pitfall (July 2026):** When a module has multiple sections and you delete a section by its end marker, the end marker is the START of the next section — so everything between the section being deleted and the next section is also removed. Always check the content between the intended end-marker and the next section before using section-boundary deletion. Webhooks live between Launch Calendar and Company Intel in ig_mentions_gaps.js; deleting Launch Calendar by targeting `// COMPANY INTEL MODAL` as the stop point accidentally removed webhooks too.
