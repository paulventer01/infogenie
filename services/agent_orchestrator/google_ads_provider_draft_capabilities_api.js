...
function graph(base,b){return {...base,draftId:b.campaign_draft_id,draftRevision:b.campaign_draft_revision,publishingRequestId:b.publishing_request_id,publishApprovalId:b.publish_approval_id,intentId:b.delivery_intent_id,credentialRefId:b.credential_reference_id,credentialRefVersion:b.credential_reference_version,googleAdsCustomerId:b.google_ads_customer_id};}
const graphFields=['campaign_draft_id','campaign_draft_revision','publishing_request_id','publish_approval_id','delivery_intent_id','credential_reference_id','credential_reference_version','google_ads_customer_id'];
router.post('/confirmations',identify,limiter,json,route('confirm',(base,req)=>{exact(req.body,[...graphFields,'final_confirmation']);return transaction(c=>capability.confirm(c,{...graph(base,req.body),finalConfirmation:req.body.final_confirmation}));}));
router.post('/',identify,limiter,json,route('issue',(base,req)=>{exact(req.body,[...graphFields,'final_confirmation_id','ttl_ms'],[...graphFields,'final_confirmation_id']);const b=req.body;return transaction(c=>capability.issue(c,{...graph(base,b),finalConfirmationId:b.final_confirmation_id,ttlMs:b.ttl_ms}));}));
