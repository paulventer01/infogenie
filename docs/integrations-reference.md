...
| **Pagination** | Live Google research is a single ads_search hop; no fake page offset. Bound continuation `{v:1,t,r,a}` is accepted only as defense against a legacy/forged cursor. Same first `creative_id` as the supplied cursor → `invalid_response` `repeated_continuation_token`. ATC `url` / `preview_url` / `preview_image` are never fetched |
...
| **Honesty** | Live pages stamp `provider_metrics.source=live` (no `_fabricated`). Empty live results stay live — never substitute the fixture page. Live Google research is a single ads_search hop; no fake page offset. ATC source URLs are syntactic HTTPS refs only |
