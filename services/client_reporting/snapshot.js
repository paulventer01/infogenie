...
async function buildReportSnapshot(db, tenantId, clientId, expectedVersion) {
  ...
  try {
    ...
    const data = await sources.data(db, profile.report_source, spec, tenantId, clientId, 0, 100);
    ...
    return { snapshot, profile, client };
  } catch (error) {
...
module.exports = { activeClient, buildReportSnapshot, CLIENT_COLUMNS, PROFILE_COLUMNS };
