...
- **HubSpot CRM connected (2026-05-04)**: `HUBSPOT_PRIVATE_APP_TOKEN` (`pat-…`, 44 chars) provides read access to contacts / deals / companies / lists. Used by server.js routes around lines 11498-11551 (e.g. `/api/hubspot/contacts`, `/api/hubspot/deals`). Smoke-tested live — `GET /crm/v3/objects/contacts` returned 200 + 2 records, deals endpoint returned 200 + 0 (account currently has no open deals).
...
- **Gemini key added but BLOCKED (2026-05-05)**: `GEMINI_API_KEY` (`AIzaSy…`, 39 chars) is valid but `generateContent` returns 429 "prepayment credits depleted". The owning Google Cloud project has billing enabled with no balance, which forces paid-tier mode and bypasses the free tier. Fix: regenerate the key in a Google Cloud project where **billing is disabled** (free tier only kicks in for billing-disabled projects), OR add prepay credits at https://ai.studio/projects.
