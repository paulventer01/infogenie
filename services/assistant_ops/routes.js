...
module.exports = function register(app, ctx) {
  ...
  const { enrichAlert, rehydrateAlert, clientMap, adminSettingsPath } = require('./services/assistant_ops/provider_links');
...
app.get('/api/alerts/list', async (req, res) => {
  ...
  res.json({
    ok:true,
    alerts: data.alerts.slice(0, limit).map(rehydrateAlert),
