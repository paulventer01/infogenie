...
*   **Share of Voice Tracker (Compete → Share of Voice, Tier 3 #1)**: Time-series share of media mentions vs competitors under `services/sov/`. Single table `sov_snapshots` (watchlist_id, target_brand, brand, mentions, pos/neu/neg counts, taken_at) populated automatically inside the Crisis Radar `_runOne()` cron — every 6h scan now writes one row per brand (target + each competitor) sharing the same `taken_at` so the chart can pivot. `GET /api/sov/series?brand=X&days=N` returns `{series:[{brand,points,total}], sov:[{brand,total,share}]}`. `GET /api/sov/targets` lists distinct tracked brands. Frontend `buildSovTracker()` renders a Chart.js stacked-area line chart + ranked share table with progress bars; the brand selected in Crisis Radar drives this view (no separate config needed).
*   **Influencer Discovery (Reach → Influencer Discovery, Tier 3 #2)**: Perplexity-powered creator finder under `services/discovery/api.js`. `POST /api/discovery/influencers {niche, platform, country, count}` calls Perplexity sonar with strict-JSON prompt asking for real currently-active creators (handle, platform, follower estimate, engagement rate, why-fit reason, source URLs). Filters out nano accounts <5k followers. Frontend `buildDiscovery()` renders compact form + creator card grid; each card has a green **+ Add to Influencer CRM** button that POSTs straight into `/api/influencers` with status='prospect' (no separate dataset — fully integrated into the existing CRM). Falls back to a placeholder card when `PERPLEXITY_API_KEY` is missing/dummy.
...
*   **AI Daily Digest (Plan → Daily Digest, Tier 4 #1)**: One-page executive summary across every Brandwatch-killer module under `services/digest/` (schema, generator, api). Single table `digest_runs` (brand, headline, summary_md, sections JSONB, stats JSONB, generated_by, delivered_to JSONB, created_at). Generator pulls last-24h crisis incidents, sov_snapshots delta vs prior 24h, recent trend_runs, new battle_cards, new influencers — composes via GPT-4o-mini with strict-JSON system prompt (`{headline, summary_md, sections:[{title,kind:warning|win|action|highlight,body}], recommended_actions[]}`); falls back to deterministic template summary when OpenAI key missing/dummy (`/^_DUMMY/i` gate). Auto-runs every 24h via `startCron(24)` for every brand in `crisis_watchlist`. `POST /api/digest/run-now`, `GET /api/digest/history`, `GET /api/digest/:id`, `POST /api/digest/:id/send {channels:['slack']}` (uses existing SSRF-guarded slack/discord webhook helper, hosts allowlisted, https-only, 8s timeout). Frontend `buildDigest()` renders compact form + current-digest gradient header (purple #7C3AED) + colour-coded section cards (warning red / win green / action amber / highlight blue) + "📤 Send to Slack" + "📋 Copy as Markdown" buttons + clickable history list.
*   **Reply Assistant (Reach → Reply Assistant, Tier 4 #2)**: Inbox-style triage of your top mentions with one-click AI-drafted replies under `services/reply_assistant/api.js`. `GET /api/reply-assistant/inbox?brand=X&days=N` loops back to existing `/api/mentions` (DataForSEO + sentiment), scores each by sentiment (negative=+3, neutral=+1, positive=+0.5) plus high-reach source bonus (Reuters/Bloomberg/NYT/X/TikTok/Reddit/etc=+2), dedups by URL, returns top 30 ranked. `POST /api/reply-assistant/draft {mention,brand,tone,ourPosition}` calls GPT-4o-mini with role-as-social-manager prompt, tone allowlist (`supportive|neutral|firm|grateful|curious`), returns `{reply, alt_short (under 240 chars for X), confidence}`; template fallback covers each sentiment when OpenAI key missing/dummy. Frontend `buildReplyAssistant()` shows brand+days form, ranked inbox cards with sentiment-coloured left border + sentiment pill + 280-char snippet + "✨ Draft Reply" + "🔗 Open source" (via `_safeUrl()` + `rel="noopener noreferrer"`); inline drawer with tone selector + optional our-position input + 📋 Copy buttons for both long + short variants.
