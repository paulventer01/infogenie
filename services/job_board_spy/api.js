...
router.get('/runs', _safeAsync(async (req, res) => {
  ...
  const r = await _db.getPool().query('SELECT id, company, total_jobs, by_dept, strategic_signals, created_at FROM job_board_runs ORDER BY created_at DESC LIMIT 30');
