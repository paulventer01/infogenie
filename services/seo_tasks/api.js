...
// De-dupes against any task for the same url+check_id whose status is not 'done'/'wont_fix'.
router.post('/import-from-audit', _safeAsync(async (req, res) => {
  ...
  const r = await _db.getPool().query(`SELECT id, url, checks FROM seo_audit_runs WHERE id=$1`, [auditRunId]);
  ...
  // Race-safe: rely on the partial unique index `uniq_seo_tasks_active` and let
  // ON CONFLICT DO NOTHING absorb concurrent imports / re-audits — `RETURNING id`
  // is empty when a duplicate active task already exists.
  let created = 0, skipped = 0;
  for (const c of issues) {
    const r2 = await _db.getPool().query(
      `INSERT INTO seo_tasks (source, source_url, check_id, label, message, fix, priority, audit_run_id)
       VALUES ('seo_audit',$1,$2,$3,$4,$5,$6,$7)
       ON CONFLICT (source_url, check_id) WHERE status NOT IN ('done','wont_fix') DO NOTHING
       RETURNING id`,
      [url, c.id, c.label, c.message, c.fix, _priorityFor(c), auditRunId]
    );
...
router.get('/stats', _safeAsync(async (req, res) => {
  ...
  const r = await _db.getPool().query(`SELECT status, COUNT(*)::int AS n FROM seo_tasks GROUP BY status`);
...
router.patch('/:id', _safeAsync(async (req, res) => {
  ...
  args.push(id);
  const sql = `UPDATE seo_tasks SET ${fields.join(', ')} WHERE id=$${args.length} RETURNING *`;
  const r = await _db.getPool().query(sql, args);
...
router.delete('/:id', _safeAsync(async (req, res) => {
  if (!_db.hasDb || !_db.hasDb()) return _err(res, 503, 'Database required.');
  await _db.getPool().query(`DELETE FROM seo_tasks WHERE id=$1`, [parseInt(req.params.id, 10)]);
  res.json({ ok: true });
}));

// Re-audit one URL and auto-close any open tasks whose check now passes.
// Returns counts so the UI can refresh.
router.post('/reaudit', _safeAsync(async (req, res) => {
  ...
  if (passingChecks.length) {
    const r = await _db.getPool().query(
      `UPDATE seo_tasks
        ...
        WHERE source_url=$1 AND check_id = ANY($2::text[]) AND status NOT IN ('done','wont_fix')
        RETURNING id`,
      [audit.url, passingChecks]
    );
    autoClosed = r.rowCount;
  }

  // Persist the new audit and import any newly-failing checks.
  let newAuditId = null;
  try {
    const r2 = await _db.getPool().query(
  ...
  // Race-safe insert via the partial unique index — see /import-from-audit comment.
  let imported = 0;
  for (const c of audit.checks.filter(c => c.status === 'warn' || c.status === 'fail')) {
    const r3 = await _db.getPool().query(
      `INSERT INTO seo_tasks (source, source_url, check_id, label, message, fix, priority, audit_run_id)
       ...
       ON CONFLICT (source_url, check_id) WHERE status NOT IN ('done','wont_fix') DO NOTHING
       RETURNING id`,
      [audit.url, c.id, c.label, c.message, c.fix, _priorityFor(c), newAuditId]
    );
