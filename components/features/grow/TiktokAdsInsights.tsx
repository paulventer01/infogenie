...
// Native React port of the legacy `tiktok-ads-insights` panel (was
// `window.buildTiktokAdsInsights` / `_ttiLoad` / `_ttiTest` in
// `public/js/ig_intel_pack_a.js` + `#view-tiktok-ads-insights` in index.html).
// Renders live TikTok Ads performance — account summary tiles, campaign table
// and top creatives — pulled from the same Express endpoints:
//   POST /api/tiktok-ads-insights/test
//   GET  /api/tiktok-ads-insights/account-summary?date_preset=…
//   GET  /api/tiktok-ads-insights/campaigns?date_preset=…
//   GET  /api/tiktok-ads-insights/top-ads?date_preset=…
...
export default function TiktokAdsInsights() {
  ...
  const load = useCallback(async (preset: string) => {
    ...
    const [s, c, a] = await Promise.all([
      apiGet<SummaryResult>(
        `/api/tiktok-ads-insights/account-summary?date_preset=${preset}`,
      ),
      apiGet<CampaignsResult>(
        `/api/tiktok-ads-insights/campaigns?date_preset=${preset}`,
      ),
      apiGet<AdsResult>(
        `/api/tiktok-ads-insights/top-ads?date_preset=${preset}`,
      ),
    ]);
    setSummary(s);
    setCamps(c);
  ...
  async function runTest() {
    setTestMsg("⏳ Pinging TikTok…");
    const r = await apiPost<TestResult>("/api/tiktok-ads-insights/test");
    setTestMsg(
      r.ok
    ...
    <div className="view-header-wrap">
          ...
          <div className="vh-inner">
            <div>
              <div className="breadcrumb">
                ...
                <span className="bc-sep">›</span> TikTok Ads Insights
              </div>
              <h2 className="view-title">🎵 TikTok Ads Insights</h2>
              <p className="view-sub">
                Live performance from your TikTok Ads account — spend, ROAS, top
                campaigns and best creatives. Pulled in real-time via the TikTok
                Marketing API.
              </p>
            </div>
          </div>
      ...
      <div className="container" style={{ paddingTop: 24, paddingBottom: 56 }}>
        <div
          style={{
            background:
              "linear-gradient(135deg,#000 0%,#25F4EE 50%,#FE2C55 100%)",
            color: "#fff",
            borderRadius: 12,
            padding: "18px 22px",
            marginBottom: 16,
        ...
        >
          <div>
            ...
            <div style={{ fontSize: "0.82rem", opacity: 0.92, marginTop: 2 }}>
              Live spend, ROAS, top campaigns and creatives — refreshes on every
              load.
            </div>
          </div>
        ...
        <div>
          ...
          {!loading && summary?.ok && !summary?.note && m && (
            ...
            >
              <Tile label="Spend" val={fmt(m.spend, "money")} color="#FE2C55" />
              <Tile
                label="Revenue"
              ...
              <Tile
                ...
                color="#25F4EE"
              />
              <Tile label="Reach" val={fmt(m.reach)} color="#25F4EE" />
              <Tile label="Clicks" val={fmt(m.clicks)} color="#25F4EE" />
              ...
              <Tile
                label="Frequency"
                val={(m.frequency ?? 0).toFixed(2)}
                color="#9CA3AF"
              />
              <Tile
                label="Conversions"
                val={fmt(m.conversions)}
                color="#15803D"
              />
            </div>
          )}
                  ...
                  <tbody>
                    {camps.campaigns.map((x, i) => (
                      <tr key={i} style={{ borderTop: "1px solid #F3F4F6" }}>
                        ...
                        <td style={TD}>{fmt(x.conversions)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
