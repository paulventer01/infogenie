...
const activation=require('./google_ads_campaign_activation');
const {createRateLimiter}=require('../security/rate_limit');
const router=express.Router(),json=express.json({limit:'2kb'});
const activationLimiter=createRateLimiter({name:'google-ads-activation-execution',windowMs:60_000,max:5,failClosed:true,
...
const status=code=>code==='human_session_required'?401:code==='permission_denied'?403:code==='capability_expired'?410:
 ['capability_rejected','authority_not_found'].includes(code)?404:['authoritative_binding_mismatch','post_review_reconciliation_required','capability_conflict','activation_conflict','activation_rejected','activation_object_binding_rejected'].includes(code)?409:400;
...
router.post('/:capabilityId/activate',activationLimiter,json,route('activate',(base,req)=>{exact(req.body,['invocation_id']);
 return activation.execute(db.getPool(),{...base,capabilityId:req.params.capabilityId,invocationId:req.body.invocation_id,allowLive:true});}));
