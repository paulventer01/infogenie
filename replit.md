...
*   **Brandwatch-Killer Suite — Tiers 1-14 (compressed index)** — full design notes in git history; each entry: route · key endpoint · storage. All follow the same pattern (strict-JSON LLM prompt + `/^_DUMMY/i` key gate + template fallback + Postgres persistence + `_escapeHtml`/`_safeUrl` frontend builder).
