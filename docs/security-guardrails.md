# InfoGenie — Security & Guardrails Scaffolding

This document describes the **foundational guardrails** that should be in place before adding new product surface area.
...
| `csrf.js` | Origin/Referer check on cookie-authenticated mutations | `SECURITY_CSRF=shadow` (default) → `on` |
| `rate_limit.js` | Sliding-window limiter; auth abuse preset | always on for `/api/auth/{login,signup,request-reset}` |
