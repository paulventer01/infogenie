...
async function _runLocalSeo(url, opts = {}) {
...
router.post('/run', _safeAsync(async (req, res) => {
  ...
  const result = await _runLocalSeo(url, { headless: !!(req.body && req.body.headless) });
  if (!result.ok) return _err(res, 400, result.error);
  if (_db.hasDb && _db.hasDb()) {
    try {
      const id = 'local_' + crypto.randomBytes(6).toString('hex');
      const tid = await _tenantCtx.resolveTenantId(req, { label: 'local_seo:run' });
      await _db.getPool().query(
        `INSERT INTO local_seo_runs (tenant_id, id, url, score, grade, summary, checks) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
