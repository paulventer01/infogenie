...
function buildBattlePlan() {
  ...
  const tabs = comps.map((comp, i) => {
    ...
    const active = i === idx;
    return `<button onclick="switchBattlePlanComp(${i})" style="display:flex;align-items:center;gap:8px;padding:10px 18px;border:none;border-bottom:3px solid ${active?'#0066FF':'transparent'};background:${active?'#EFF6FF':'transparent'};cursor:pointer;color:${active?'#0A1628':'#374151'};font-size:0.8rem;font-weight:${active?700:500};white-space:nowrap;font-family:'Inter',sans-serif;transition:all .15s">
    ...
    <div class="bp-comp-summary" style="background:#FFFFFF;border-bottom:1px solid #E8EEF8;padding:14px 28px;box-shadow:0 2px 8px rgba(10,20,50,.05)">
      <div style="max-width:1200px;margin:0 auto;display:flex;align-items:center;gap:20px;flex-wrap:wrap">
        ...
        <div style="flex:1;display:flex;gap:24px;flex-wrap:wrap">
          <div style="text-align:center" title="Estimated total monthly website visits for ${c.name} — organic + paid combined."><div style="font-size:0.92rem;font-weight:800;color:#0A1628">${traffic}</div><div style="font-size:0.62rem;color:#475569;text-transform:uppercase;letter-spacing:.06em">Traffic/mo</div></div>
          <div style="text-align:center" title="Click-Through Rate: % of ad impressions that result in a click. This competitor's average across all campaigns."><div style="font-size:0.92rem;font-weight:800;color:#0A1628">${c.ctr||'—'}</div><div style="font-size:0.62rem;color:#475569;text-transform:uppercase;letter-spacing:.06em">CTR</div></div>
          <div style="text-align:center" title="Return on Ad Spend — estimated revenue earned per $1 spent on ads by this competitor."><div style="font-size:0.92rem;font-weight:800;color:#0A1628">${c.roas||'—'}×</div><div style="font-size:0.62rem;color:#475569;text-transform:uppercase;letter-spacing:.06em">ROAS</div></div>
          <div style="text-align:center" title="Estimated monthly advertising budget across Google, Meta, TikTok and other paid channels."><div style="font-size:0.92rem;font-weight:800;color:#0A1628">${c.adSpend||'—'}</div><div style="font-size:0.62rem;color:#475569;text-transform:uppercase;letter-spacing:.06em">Ad Spend</div></div>
          <div style="text-align:center" title="The marketing channel where this competitor is investing the most budget and generating the best results."><div style="font-size:0.92rem;font-weight:800;color:#0A1628">${c.topChannel||'—'}</div><div style="font-size:0.62rem;color:#475569;text-transform:uppercase;letter-spacing:.06em">Top Channel</div></div>
