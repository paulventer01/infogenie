...
interface CompetitorPrediction {
  ...
  evidence: string[]; recommended_action: string; watch_signals: string[]; _meta?: PredictionMeta;
}
interface CampaignPrediction {
  ...
  monthly_breakdown: MonthlyBreakdown[]; _meta?: PredictionMeta;
}
...
interface RevenuePrediction {
  ...
  scenario_data: MonthlyBreakdown[]; _meta?: PredictionMeta;
}
...
function PanelShell({ icon, title, color, children }: { icon: string; title: string; color: string; children: React.ReactNode }) {
  return (
    <div style={{
      ...
      <h2 style={{ margin: 0, fontSize: ".95rem", fontWeight: 800, color: "var(--text, #111827)", display: "flex", alignItems: "center", gap: 8 }}>
...
// ─── Revenue Scenario Chart (pure CSS/SVG, no external Chart.js dep) ──────
  ...
  return (
    <div style={{ marginTop: 4 }}>
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }}>
        ...
        <text x={PAD - 4} y={ys(exp)[0]} fontSize={9} fill="var(--text-muted,#9CA3AF)" textAnchor="end">{fmt(exp[0])}</text>
...
function CompetitorPanel({ state }: { state: PanelState<CompetitorPrediction> }) {
  ...
  return (
    <PanelShell icon="🏆" title="Competitor Move Forecast" color="#EF4444">
      {loading ? <Skeleton /> : error ? <ErrorState msg={error} /> : !data ? null : (
        <>
          ...
          <div>
            <p style={{ margin: "0 0 4px", fontSize: ".72rem", fontWeight: 700, color: "var(--text-muted,#6B7280)", textTransform: "uppercase", letterSpacing: ".05em" }}>Evidence</p>
...
function CampaignPanel({ state }: { state: PanelState<CampaignPrediction> }) {
  const { data, loading, error } = state;
  return (
    <PanelShell icon="🚀" title="Campaign Outcome Forecast" color="#3B82F6">
      {loading ? <Skeleton /> : error ? <ErrorState msg={error} /> : !data ? null : (
        <>
          <ConfidenceBadge score={data.confidence} />
          <p style={{ margin: 0, fontSize: ".82rem", color: "var(--text, #374151)", lineHeight: 1.6 }}>{data.summary}</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
            {[
              ...
              <div key={label} style={{ textAlign: "center", padding: "10px 6px", background: color + "0F", borderRadius: 8, border: `1px solid ${color}30` }}>
                ...
                <div style={{ fontSize: "1.1rem", fontWeight: 800, color: "var(--text,#111827)" }}>
...
function ChurnPanel({ state }: { state: PanelState<ChurnPrediction> }) {
  const { data, loading, error } = state;
  return (
    <PanelShell icon="⚠️" title="Churn Trajectory" color="#F59E0B">
      {loading ? <Skeleton /> : error ? <ErrorState msg={error} /> : !data ? null : (
        <>
          <ConfidenceBadge score={data.confidence} />
          <p style={{ margin: 0, fontSize: ".82rem", color: "var(--text, #374151)", lineHeight: 1.6 }}>{data.summary}</p>
          ...
          {(data.retention_levers || []).length > 0 && (
            <div>
              <p style={{ margin: "0 0 4px", fontSize: ".72rem", fontWeight: 700, color: "var(--text-muted,#6B7280)", textTransform: "uppercase", letterSpacing: ".05em" }}>Retention Levers</p>
...
function RevenuePanel({ state }: { state: PanelState<RevenuePrediction> }) {
  const { data, loading, error } = state;
  return (
    <PanelShell icon="💹" title="90-Day Revenue Scenarios" color="#10B981">
      {loading ? <Skeleton /> : error ? <ErrorState msg={error} /> : !data ? null : (
        <>
          <ConfidenceBadge score={data.confidence} />
          <p style={{ margin: 0, fontSize: ".82rem", color: "var(--text, #374151)", lineHeight: 1.6 }}>{data.summary}</p>
          ...
          {(data.growth_levers || []).length > 0 && (
            <div>
              <p style={{ margin: "0 0 4px", fontSize: ".72rem", fontWeight: 700, color: "var(--text-muted,#6B7280)", textTransform: "uppercase", letterSpacing: ".05em" }}>Growth Levers</p>
...
export default function PredictiveIntelligence() {
  ...
  async function fetchPanel<T>(
    ...
    try {
      const r = await fetch(url, {
        ...
        body: JSON.stringify({ ...body, ...(force ? { force: true } : {}) }),
      });
  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "6px 0 0", fontSize: ".85rem", color: "var(--text-muted, #6B7280)" }}>
            AI-powered forecasts for competitor moves, campaign outcomes, churn trajectory, and 90-day revenue — refreshed every 6 hours.
      ...
      {/* Empty state */}
      {!anyLoading && !competitor.data && !campaign.data && !churn.data && !revenue.data && (
        <div style={{
