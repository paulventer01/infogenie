...
// Native React port of the legacy `meta-insights` panel (was
// `window.buildMetaInsights` / `_miLoad` / `_miTest` in
// `public/js/ig_intel_pack_a.js` + `#view-meta-insights` in index.html).
// Renders live Meta Ads performance — account summary tiles, campaign table and
// top creatives — pulled from the same Express endpoints:
//   POST /api/meta-insights/test
//   GET  /api/meta-insights/account-summary?date_preset=…
//   GET  /api/meta-insights/campaigns?date_preset=…
//   GET  /api/meta-insights/top-ads?date_preset=…
...
export default function MetaInsights() {
  ...
  const load = useCallback(async (preset: string) => {
    ...
    const [s, c, a] = await Promise.all([
      apiGet<SummaryResult>(
        `/api/meta-insights/account-summary?date_preset=${preset}`,
      ),
      apiGet<CampaignsResult>(
        `/api/meta-insights/campaigns?date_preset=${preset}`,
      ),
      apiGet<AdsResult>(`/api/meta-insights/top-ads?date_preset=${preset}`),
    ]);
    setSummary(s);
    setCamps(c);
    setAds(a);
  ...
  async function runTest() {
    setTestMsg("⏳ Pinging Meta…");
    const r = await apiPost<TestResult>("/api/meta-insights/test");
    setTestMsg(
      r.ok
          ...
          <div className="vh-inner">
            <div>
              <div className="breadcrumb">
                ...
                <span className="bc-sep">›</span> Meta Ads Insights
              </div>
              <h2 className="view-title">📘 Meta Ads Insights</h2>
              <p className="view-sub">
                Live performance from your Meta Ads account — spend, ROAS, top
                campaigns and best-performing creatives. Pulled in real-time via
          ...
          {!loading && summary?.ok && !summary?.note && m && (
            ...
            >
              ...
              <Tile
                ...
                color="#0EA5E9"
              />
              <Tile label="Reach" val={fmt(m.reach)} color="#0EA5E9" />
                  ...
                  <tbody>
                    {camps.campaigns.map((x, i) => (
                      <tr key={i} style={{ borderTop: "1px solid #F3F4F6" }}>
                        ...
                        <td style={TD}>{fmt(x.purchases)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
