...
async function _runEnsureAgentOrchestratorSchemaLocked(p) {
  ...
  await p.query(`
    CREATE TABLE IF NOT EXISTS orchestrator_google_ads_activation_attempts(
      ...
      requires_reconciliation BOOLEAN NOT NULL DEFAULT FALSE,
      external_action_taken BOOLEAN NULL,
      ...
      FOREIGN KEY(tenant_id,operation_id) REFERENCES orchestrator_google_ads_provider_draft_operations(tenant_id,id) ON DELETE RESTRICT,
      FOREIGN KEY(tenant_id,reconciliation_run_id) REFERENCES orchestrator_google_ads_reconciliation_runs(tenant_id,id) ON DELETE RESTRICT,
      CONSTRAINT orchestrator_gaact_hashes CHECK(session_id_hash~'^[0-9a-f]{64}  // identifiers, credential references, source hashes, payloads, or errors are stored.
  // orchestrator_advertising_global_kill_switches is a platform-wide GLOBAL
...
module.exports = {
  ...
  _tryAdvisoryLockUntil,
        ...
        AND account_fingerprint~'^[0-9a-f]{64}  // identifiers, credential references, source hashes, payloads, or errors are stored.
  // orchestrator_advertising_global_kill_switches is a platform-wide GLOBAL
  // singleton (PK switch_key). It must never receive tenant_id.
  await p.query(`
    CREATE TABLE IF NOT EXISTS orchestrator_advertising_global_kill_switches(
...
module.exports = {
 ...
 AND ledger_root_hash~'^[0-9a-f]{64}  // identifiers, credential references, source hashes, payloads, or errors are stored.
  // orchestrator_advertising_global_kill_switches is a platform-wide GLOBAL
  // singleton (PK switch_key). It must never receive tenant_id.
  await p.query(`
    CREATE TABLE IF NOT EXISTS orchestrator_advertising_global_kill_switches(
...
module.exports = {
  ...
  _tryAdvisoryLockUntil,
        ...
        AND objects_digest~'^[0-9a-f]{64}  // identifiers, credential references, source hashes, payloads, or errors are stored.
  // orchestrator_advertising_global_kill_switches is a platform-wide GLOBAL
  // singleton (PK switch_key). It must never receive tenant_id.
  await p.query(`
    CREATE TABLE IF NOT EXISTS orchestrator_advertising_global_kill_switches(
...
module.exports = {
 ...
 AND invocation_id_hash~'^[0-9a-f]{64}  // identifiers, credential references, source hashes, payloads, or errors are stored.
  // orchestrator_advertising_global_kill_switches is a platform-wide GLOBAL
  // singleton (PK switch_key). It must never receive tenant_id.
  await p.query(`
    CREATE TABLE IF NOT EXISTS orchestrator_advertising_global_kill_switches(
  ...
  _tryAdvisoryLockUntil,
    ...
    DECLARE cap RECORD; object_count INTEGER; BEGIN
      ...
      IF TG_OP='INSERT' THEN
        ...
        IF NOT FOUND OR cap.status<>'consumed' OR object_count<>3
