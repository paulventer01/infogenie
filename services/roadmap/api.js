...
router.get('/progress', async (_req, res) => {
  ...
  const r = await _db.getPool().query(`SELECT task_id, completed_at FROM roadmap_progress`);
  const completed = {};
...
router.delete('/progress/:taskId', async (req, res) => {
  if (!_db.hasDb()) return res.status(503).json({ ok: false, error: 'no-db' });
  const taskId = String(req.params.taskId).slice(0, 64);
  await _db.getPool().query(`DELETE FROM roadmap_progress WHERE task_id = $1`, [taskId]);
  res.json({ ok: true });
});
