...
async function data(db, name, spec, tenantId, clientId, cursor, limit) {
  ...
  // Every child joins both the tenant-scoped canonical root and current mapping.
  ...
  if (name === 'search-intel') {
    const totals = await db.query(`SELECT count(*)::int AS runs,
      ...
      ${childJoin('search_intel_llm_runs')}`, [tenantId, clientId]);
    const runs = await db.query(`SELECT c.id,c.query_id,c.provider,c.brand_mentioned,c.brand_position,c.ran_at,
      (c.error IS NOT NULL) AS failed ${childJoin('search_intel_llm_runs')}
      ORDER BY c.ran_at DESC,c.id DESC LIMIT 50`, [tenantId, clientId]);
    summary = { ...count.rows[0], ...totals.rows[0] };
    recent = { llm_runs: runs.rows };
  } else {
    // Currency groups prevent adding unlike monetary units into a false total.
    const totals = await db.query(`SELECT r.currency,count(*)::int AS performance_rows,
      ...
      sum(c.conversions) AS conversions,sum(c.revenue) AS revenue ${childJoin('ad_performance_hourly')}
      GROUP BY r.currency ORDER BY r.currency`, [tenantId, clientId]);
    const performance = await db.query(`SELECT c.id,c.campaign_id,r.currency,c.bucket_hour,c.spend,c.impressions,
      c.clicks,c.conversions,c.revenue ${childJoin('ad_performance_hourly')}
      ORDER BY c.bucket_hour DESC,c.id DESC LIMIT 50`, [tenantId, clientId]);
    const actions = await db.query(`SELECT c.id,c.campaign_id,c.action_type,c.applied,c.created_at
      ${childJoin('optimizer_actions')} ORDER BY c.created_at DESC,c.id DESC LIMIT 50`, [tenantId, clientId]);
  ...
  return { source: name, ...page(roots.rows, limit), summary, recent, recent_limit: 50,
    summary_scope: 'all_mapped_records', excluded_sections: spec.excluded };
}
