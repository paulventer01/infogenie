...
const status=code=>code==='human_session_required'?401:code==='permission_denied'?403:
  ...
  ['activation_attempt_ineligible','authoritative_binding_mismatch','idempotency_conflict','invalid_reconciliation_transition'].includes(code)?409:400;
...
function route(label,fn){return async(req,res)=>{try{if(!human(req))throw Object.assign(new Error(),{code:'human_session_required'});
 ...
 }catch(e){return res.status(status(e.code)).json({error:e.code||'reconciliation_request_failed',external_action_taken:false});}};}
