...
if(!db.hasDb())test('PR8B PostgreSQL requires DATABASE_URL',{skip:'no DATABASE_URL'},()=>{});else{
 ...
 test('lifecycle approves with distinct human, rejects, replays decisions, and protects versions and terminals',async()=>{
  ...
  const done=await move(f,r,f.approver,'approve','approve',{note:'Human approval for later internal execution review.'}),replay=await move(f,r,f.approver,'approve','approve',{note:'Human approval for later internal execution review.'});assert.deepEqual(replay,done);assert.equal(done.state,'approved');assert.equal(Number.isSafeInteger(done.deciding_user_id),true);assert.equal(done.deciding_user_id,f.approver);
  await assert.rejects(move(f,done,f.approver,'reject','terminal'),{code:'invalid_transition'});await assert.rejects(db.getPool().query(`DELETE FROM orchestrator_optimization_execution_requests WHERE tenant_id=$1 AND id=$2`,[f.tenant,done.request_id]),{message:'orchestrator_oer_delete_prohibited'});
