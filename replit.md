...
- **Google Custom Search key resolution** (server.js ~2155 `_resolveGoogleCloudKey`, used at ~2928 + ~2945): the `GOOGLE_SEARCH_API_KEY` slot is shared with a RapidAPI Google Search wrapper, so the official Custom Search code path validates each candidate (`startsWith('AIza')` after trim) and returns `''` when no Google-shaped key is present. Falls back to `GOOGLE_PAGESPEED_API_KEY` (same Google Cloud project's key works for both APIs). Prevents accidental leakage of a non-Google key into `googleapis.com` URLs. Custom Search currently parked — see Project-status note below.
- **Project-status note (Google Custom Search)**: enabled API + valid AIza key in project `infogenie-492811`, but `customsearch.googleapis.com` returns 403 "project does not have access". Likely Google Cloud project/billing edge case. Parked 2026-05-04 — DataForSEO covers all real SERP work; RapidAPI Google Search wrapper still works via `RAPIDAPI_KEY` fallback. Re-enable later by ensuring `GOOGLE_SEARCH_API_KEY` (or `GOOGLE_PAGESPEED_API_KEY`) is in a project with Custom Search JSON API + billing linked.
- **Resend email — production setup complete (2026-05-04)**: `RESEND_FROM_EMAIL=Suretrade <alerts@suretrade.co.za>` (domain DKIM/SPF verified in Resend dashboard) so alert digests + drip emails now ship from the user's own brand domain instead of `onboarding@resend.dev`. `RESEND_WEBHOOK_SECRET` set to a `whsec_…` Svix signing secret; webhook endpoint `POST /api/drips/webhook/resend` (server.js ~6187) verifies via `_verifyResendWebhook` — Svix-style HMAC over `${id}.${ts}.${rawBody}`, 5-minute timestamp window, timing-safe comparison; rejects with 401 when verification fails. Subscribed events: sent / delivered / bounced / complained / opened / clicked. **Webhook URL is currently the dev replit.dev domain** — when InfoGenie is deployed, update the endpoint URL in Resend dashboard to the `.replit.app` (or custom) production domain to keep deliverability metrics flowing.
- **HubSpot CRM connected (2026-05-04)**: `HUBSPOT_PRIVATE_APP_TOKEN` (`pat-…`, 44 chars) provides read access to contacts / deals / companies / lists. Used by server.js routes: `GET /api/hubspot/status` (~11497), `POST /api/hubspot/contacts/upsert` (~11511), `GET /api/hubspot/deals` (~11550). Smoke-tested live via direct `crm/v3/objects` calls — contacts returned 200 + 2 records, deals returned 200 + 0 (account currently has no open deals).
- **Firecrawl connected (2026-05-04)**: `FIRECRAWL_API_KEY` (`fc-…`) used as the bot-block fallback for Content Scorer scrapes (server.js ~8045). Triggered automatically only when normal `fetch` returns 403 / 429 / 503, so free-tier quota (500 pages/mo) is preserved for sites that genuinely need it (Trustpilot, LinkedIn, Facebook, Glassdoor, Amazon, etc.). Direct endpoint `POST /api/firecrawl/scrape` exposed at server.js ~11571.
- **Perplexity connected (2026-05-05)**: `PERPLEXITY_API_KEY` (`pplx-…`, 53 chars). Adds real-time web research with citations to InfoGenie's deep-research + competitor-intel paths. Live-tested with `sonar` model on suretrade.co.za prompt → 200 OK with 6 citations and accurate company description.
- **Gemini connected (2026-05-05)**: `GEMINI_API_KEY` (`AIzaSy…`, 39 chars). Initial 429 "prepayment credits depleted" cleared after user added prepay credit; live-tested `gemini-2.5-flash:generateContent` → 200 OK. Paid tier (~$0.30 / M tokens on flash). Adds price diversity + redundancy to the OpenAI / Anthropic / Perplexity LLM stack.
- **GA4 / Search Console PARKED (2026-05-05)**: service account `infogenie-analytics@infogenie-492811.iam.gserviceaccount.com` mints OAuth tokens successfully (verified live), but both GSC and GA4 reject it with "This email doesn't match a Google Account" in the Add User UI. Root cause: `nexuspro.global` Workspace org policy blocks external (cross-org) identities. Diagnosed across both incognito + account-level add attempts. Re-enable later via Workspace Admin → Cloud Identity / Domain restricted sharing policy, OR by recreating the SA inside the same Workspace org.
- **⚠️ Production hardening TODO (2026-05-05)**: now that paid-tier secrets are populated (HubSpot CRM token, Firecrawl quota, Perplexity, Gemini, OpenAI, Anthropic, RapidAPI, DataForSEO), the secret-backed routes are still un-authenticated and can be invoked by anyone who knows the URL. Before deploying to a public domain, add (a) session/API-key auth on `/api/hubspot/*`, `/api/firecrawl/*`, `/api/ai-visibility-multi`, `/api/deep-research`, all model-calling routes, and (b) per-IP + per-day budget caps on Gemini / Perplexity / Firecrawl to limit blast radius from quota abuse. Diagnosed by architect review on 2026-05-05.
- **✅ Production hardening DONE (2026-05-05)**:
  - **Auth gate** (server.js ~95-122): `INFOGENIE_API_KEY` env-var-controlled middleware. When unset → no-op (dev mode). When set → all `/api/*` routes require key via `Authorization: Bearer …`, `X-InfoGenie-Key` header, or `?key=` (GET only). Allowlisted: `/api/auth/*`, `/api/drips/webhook/resend` (HMAC-signed), `/api/notify/send` (already secret-gated), `/api/status`, `/api/*/status`, `/api/budget/status`. Sets `app.set('trust proxy', true)` so `req.ip` resolves to real client IP behind Replit proxy.
  - **Budget caps** (server.js ~124-200): per-provider, per-IP + per-day daily counters with UTC midnight reset. Default 50/IP, 2000/global per day. Override via `BUDGET_LIMIT_<PROVIDER>_IP` / `_GLOBAL` (e.g. `BUDGET_LIMIT_GEMINI_GLOBAL=5000`). Wired into Firecrawl scrape + Gemini + Perplexity + Cloudflare AI in `ai-visibility-multi`. Also enforced only when `INFOGENIE_API_KEY` is set (paired with auth). `GET /api/budget/status` returns live counters.
  - **TO ENABLE IN PROD**: set `INFOGENIE_API_KEY` as a Replit secret to a long random string (`openssl rand -hex 32`), then update Resend webhook URL to the deployed `.replit.app` domain. The `/api/drips/webhook/resend` allowlist keeps Resend signed callbacks working.
- **✅ Postgres KV store DONE (2026-05-05)**: created Replit-managed PostgreSQL DB (`DATABASE_URL` auto-set) and added `db.js` module with a generic `kv_store(key text PK, value jsonb, updated_at timestamptz)` table — replaces flat-file persistence with a single, transactional, concurrent-safe schema. `migrateJsonFilesIfNeeded()` runs at server boot (server.js ~8365) and one-shot copies all 8 `data/*.json` blobs into rows keyed `file:<basename>`. Idempotent — skips already-migrated keys. File mirror still written on save so legacy sync read paths keep working. `GET /api/db/status` reports mode (`postgres` vs `file`) + row count. Initial migration moved 8 files (stakeholders, webhooks, goals, launches, qualified-leads, reengagement-campaigns, drip-enrollments, aivis-history) into the DB. Per-file load/save call sites can incrementally swap to `db.readJsonFileOrDb()` / `writeJsonFileOrDb()` as touched — no big-bang rewrite needed.
- **✅ Cloudflare Workers AI DONE (2026-05-05)**: `CLOUDFLARE_ACCOUNT_ID` + `CLOUDFLARE_AI_TOKEN` configured. Wired into `/api/ai-visibility-multi` as the 5th live LLM (server.js ~3087), replacing the empty `pending('llama')` slot. Model: `@cf/meta/llama-3.1-8b-instruct` — Llama 3.1 8B at ~$0.011 / M neurons (≈30× cheaper than Gemini), with 10k neurons/day on free tier. `GET /api/cloudflare/status` reports configured state. Live-tested: returns 200 with prompt response + token usage. Adds redundancy + cost diversity to the OpenAI / Anthropic / Perplexity / Gemini / Llama LLM stack.
