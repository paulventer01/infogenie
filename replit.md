...
*   **Brandwatch-Killer Suite (T1-T19)** — see `docs/tiers.md` for full per-feature index (route · endpoint · storage). All tiers follow the same pattern: strict-JSON LLM prompt + `/^_DUMMY/i` key gate + template fallback + Postgres persistence + `_escapeHtml`/`_safeUrl` frontend builder. One-line summary per tier:
    ...
    *   **T38** Viral Carousel Generator (`services/carousel/{schema,api}.js` · `carousels` table · 4 STRUCTURES = pure-info / storytelling / problem-solution / listicle, each with a 10-slot 10-slide template following the **Hook → Context → Value → Action** framework · OpenAI gpt-4o-mini strict-JSON with DUMMY gate + template fallback · routes `/structures`, `/generate`, `/list`, `/:id`, `DELETE /:id` · UI at `/#carousel` under Create nav with structure picker, brand-voice/audience inputs, 10 colour-graded slide cards, "📋 Copy all" + "📤 Open Social Publisher" + "📅 Add to Calendar" handoffs, recent-history list · pairs with Reddit Pulse "💡 Discover Questions Customers Ask" panel which calls `POST /api/reddit-pulse/discover-questions` (pure Reddit search across 4 query variants `?`/`how to`/`best`/`help`, question-title filter, scored by upvotes+comments×2, "→ Make Carousel" button pre-fills the topic field))
    *   **T37** Get-Started Roadmap (`services/roadmap/{schema,catalog,api}.js` · `roadmap_progress` table keyed by stable `task_id` · 90 day-by-day tasks split SEO/SMM/PPC × 30 days each, every task deep-links to the InfoGenie view that automates it via `navigateTo(view)` · weekly social cadence cards (Instagram/Facebook/LinkedIn) from the lead-magnet schedule + 5 PLAN/CREATE/ENGAGE/ANALYZE/GROW principles · progress bar in hero, filter pills SEO/SMM/PPC/All, ↻ Reset button · `GET /catalog`, `GET /progress`, `POST /progress/:taskId`, `DELETE /progress/:taskId`, `POST /reset` · nav link "🗺️ Get Started Roadmap" sits directly under Dashboard so beginners find it first)
